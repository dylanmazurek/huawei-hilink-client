...
export async function login(url: string, user: string, password: string|undefined) {
  var hilinkLogin = await checkLogin(url, user, password);
  if (!hilinkLogin) {
    ...
    const resp = await restCalls.sendDataRaw(
      `http://${url}/api/user/challenge_login`,
      'POST',
    ...
    console.log('scarmSalt', scarmSalt);
